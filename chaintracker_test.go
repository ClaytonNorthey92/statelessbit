...
func TestNewChainTracker(t *testing.T) {
	client := &rpcclient.Client{}

	chaintracker, err := NewChainTracker(client)
	if err != nil {
		t.Fatal(err)
	}

	if client != chaintracker.client {
		t.Fatalf("unexpected client: %v != %v", client, chaintracker.client)
	}
}
	...
	for _, tti := range testTable {
		...
		t.Run(tti.name, func(t *testing.T) {
			ts := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
				...
				jsonBodyBytes, err := io.ReadAll(r.Body)
				if err != nil {
					t.Fatal(err)
				}

				var requestBodyJson requestJson
				if err := json.Unmarshal(jsonBodyBytes, &requestBodyJson); err != nil {
					t.Fatal(err)
				}
				...
				if requestBodyJson.Method == "getchaintips" {
					result := btcjson.GetChainTipsResult{
						Height:    0,
						Hash:      tti.chain[0].Header.BlockHash().String(),
						BranchLen: 0,
						Status:    "active",
					}

					b, err := json.Marshal(&responseBodyJson{
				...
				if requestBodyJson.Method == "getblock" {
					for _, block := range tti.chain {
						t.Logf("checking hashes: %s ?= %s", block.Header.BlockHash().String(), requestBodyJson.Params[0])
						if block.Header.BlockHash().String() == requestBodyJson.Params[0] {
							t.Logf("found block with hash %s and prev hash %s", block.Header.BlockHash(), block.Header.PrevBlock)
							...
							respStr := hex.EncodeToString(buf.Bytes())

							b, err := json.Marshal(&responseBodyJson{Result: respStr})
							if err != nil {
								t.Fatal(err)
							}
				...
				fmt.Fprintf(w, "unexpected method: %s", requestBodyJson.Method)
			}))
			...
			ct, err := NewChainTracker(client)
			if err != nil {
				t.Fatal(err)
			}

			foundBlocks := []*wire.MsgBlock{}

			if err := ct.forEachRawBlock(func(b *wire.MsgBlock) error {
			...
			if diff := deep.Equal(foundBlocks, tti.chain); len(diff) > 0 {
